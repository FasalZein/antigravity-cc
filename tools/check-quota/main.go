// check-quota - Check Antigravity quota directly from Google's API
// Usage:
//   go run . [--web] [--port 8318]
//
// Modes:
//   CLI (default): Displays quota in terminal with colored output
//   Web (--web):   Starts HTTP server with Tailwind CSS dashboard
//
// This tool reads Antigravity auth files from ~/.cli-proxy-api/
...
// QuotaInfo contains the actual quota data
...
func main() {
	flag.BoolVar(&webMode, "web", false, "Start web server with dashboard")
	flag.IntVar(&port, "port", 8318, "Port for web server")
	flag.BoolVar(&openBrowser, "open", true, "Open browser automatically (web mode)")
	flag.Parse()
...
func runCLI() {
	...
	if err != nil {
		fmt.Printf("Error reading auth directory %s: %v\n", authDir, err)
		os.Exit(1)
	}
...
func getQuotaData() *DashboardData {
	cacheMu.RLock()
	if cachedData != nil && time.Since(cacheTime) < cacheTTL {
